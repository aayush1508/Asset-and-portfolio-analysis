...
def get_indian_m_ret(company = "ajanta") :
    """
    Load the market returns for various Indian Listed companies 
    including small cap,mid-cap and large-cap.
    """
    p = pd.read_csv(f"data/{company}.csv", header = 0, index_col = 0,
                   parse_dates=True)
    p = p[["Adj Close"]]
    r = p.pct_change()
    r.columns = [str(company)]
    r.index = pd.to_datetime(r.index, format="%Y%m").to_period('M')
    return r.dropna()
def merge_indian_m_ret() :
    ril = get_indian_m_ret("reliance")
    tcs = get_indian_m_ret("tcs")
    ajanta = get_indian_m_ret("ajanta")
    deepak = get_indian_m_ret("deepak")
    delta = get_indian_m_ret("delta")
    hdfc = get_indian_m_ret("hdfc")
    a = pd.merge(tcs, ril, left_index=True, right_index=True)
    a = pd.merge(a, hdfc, left_index=True, right_index=True)
    a = pd.merge(a, ajanta, left_index=True, right_index=True)
    a = pd.merge(a, deepak, left_index=True, right_index=True)
    a = pd.merge(a, delta, left_index=True, right_index=True)
    return a
...
import scipy.stats #import jarque bera 
...
def summary_stats(r, riskfree_rate=0.03):
    ...
    wealth = r.aggregate(lambda r: drawdown(r).Wealth[-1].round(2))
    return pd.DataFrame({
